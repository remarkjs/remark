...
function tokenizeBlockquote(eat, $0) {
    ...
    add({
        'type': 'blockquote',
        'children': self.tokenize($0.replace(EXPRESSION_BLOCK_QUOTE, ''))
    });
...
function tokenizeLinkDefinition(eat, $0, $1, $2, $3) {
    this.shared.links[$1.toLowerCase()] = eat($0)({}, {
        'type': null,
        'href': $2,
        'title': $3
    });
...
function tokenizeFootnoteDefinition(eat, $0, $1, $2, $3) {
    ...
    token.children = self.tokenize($3.replace(EXPRESSION_INITIAL_TAB, ''));

    self.inBlockquote = wasInBlockquote;

    id = $2.toLowerCase();

    self.shared.footnotes[id] = token.children;
    self.shared.footnotesAsArray.push(id);
}
 ...
 * @constructor BlockLexer
 */
function BlockLexer(options, shared, inline) {
    var self,
        rules;
 ...
 * @return {Array.<Object>}
 */
BlockLexer.prototype.lex = function (value) {
    var self;
...
BlockLexer.prototype.tokenizers = {
 ...
 * @return {Array.<Object>}
 */
BlockLexer.prototype.tokenize = function (value) {
    var self,
        tokens,
        methods,
        index,
 ...
 */
function InlineLexer(options, shared) {
    var self,
        rules;
...
function link(isLink, href, text, title) {
    ...
    if (isLink) {
        ...
        token.href = href;
        token.children = this.tokenize(text);
...
function tokenizeReferenceLink(eat, $0, $1, $2) {
    ...
    url = shared.links[text.toLowerCase()];

    if (
        self.options.footnotes &&
        text.charAt(0) === CARET &&
        shared.footnotes[text.substr(1)]
    ) {
        /*
        ...
        if (
            ...
            while (self.footnoteCounter in shared.footnotes) {
                self.footnoteCounter++;
            }

            footnote = String(self.footnoteCounter);

            shared.footnotes[footnote] = [{
                'type': 'paragraph',
                'children': text.substr(1)
            }];

            shared.footnotesAsArray.push(footnote);
 ...
 * @return {Array.<Object>}
 */
InlineLexer.prototype.tokenize = function (value) {
    var self,
        tokens,
        methods,
        index,
...
function Parser(options) {
    ...
    self.inline = new InlineLexer(options, shared);
    self.block = new BlockLexer(options, shared, self.inline);
}
...
Parser.prototype.parse = function (value) {
    ...
    token = {
        'type': 'root',
        'children': self.tokenize(self.block.lex(value))
    };

    if (self.options.footnotes) {
        footnotes = self.shared.footnotes;
        footnotesAsArray = self.shared.footnotesAsArray;

        index = -1;

        while (footnotesAsArray[++index]) {
            footnote = footnotesAsArray[index];
            footnotes[footnote] = self.tokenize(footnotes[footnote]);
        }
 ...
 * @return {Array.<Object>}
 */
Parser.prototype.tokenize = function (tokens) {
    var self,
        index,
        length,
        out;
    ...
    while (++index < length) {
        out[index] = self.tok(tokens[index]);
    }
...
Parser.prototype.tok = function (token) {
    ...
    if (type === 'text') {
        ...
        while (++index < length) {
            ...
            while (++cellIterator < cellLength) {
                cells[cellIterator].children =
                    self.inline.tokenize(cells[cellIterator].children);
            }
        }
    } else if (
        type === 'heading' ||
        ...
        token.children = self.inline.tokenize(token.children);
    } else if (
        type === 'blockquote' ||
        type === 'list' ||
        type === 'listItem'
    ) {
        token.children = self.tokenize(token.children);
    }
