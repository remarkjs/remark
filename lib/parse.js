...
function tokenizeLineHeading(eat, $0, $1, $2) {
    eat($0)(this.heading($1, $2 === EQUALS ? 1 : 2));
}
...
function tokenizeHorizontalRule(eat, $0) {
    eat($0)(this.voidToken('horizontalRule'));
}
...
function tokenizeBlockquote(eat, $0) {
    ...
    $1 = self.tokenizeBlock($0.replace(EXPRESSION_BLOCK_QUOTE, ''));

    eat($0)(this.blockToken('blockquote', $1));
...
function tokenizeList(eat, $0, $1, $2) {
    ...
    match = $0.match(self.blockRules.item);
    ...
    while (++index < length) {
        ...
        add(list, {
            'type': 'listItem',
            'loose': Boolean(loose),
            'children': self.tokenizeBlock(token)
        });
    }
...
function tokenizeFootnoteDefinition(eat, $0, $1, $2, $3) {
    ...
    token = eat($0)({}, {
        'type': null,
        'id': $2.toLowerCase(),
        'children': self.tokenizeBlock($3.replace(EXPRESSION_INITIAL_TAB, ''))
    });

    self.inBlockquote = wasInBlockquote;

    self.footnotes[token.id] = token.children;
    self.footnotesAsArray.push(token.id);
}
...
function tokenizeTable(eat, $0, $1, $2, $3, $4, $5) {
    ...
    eat($1)(token, this.blockToken('tableHeader', $2));
    ...
    while (++index < length) {
        ...
        add(token, this.blockToken('tableRow', value));
    }
    ...
    while (++index < length) {
        ...
        while (++rowIndex < rowLength) {
            row[rowIndex] = add({}, this.blockToken(
...
function tokenizeParagraph(eat, $0) {
    ...
    eat($0)(this.blockToken('paragraph', $0));
}
 ...
 * @return {Object}
 */
function blockToken(type, children) {
    return {
        'type': type,
        'children': children
    };
    ...
    if (
        ...
        if (
            ...
            self.footnotes[footnote] = [
                this.blockToken('paragraph', text.substr(1))
            ];
...
Parser.prototype.parse = function (value) {
    ...
    token = self.blockToken('root', value);
    ...
    if (self.options.footnotes) {
        ...
        while (footnotesAsArray[++index]) {
            ...
            footnotes[footnote] = self.tokenizeAll(footnotes[footnote]);
        }
...
Parser.prototype.tokenizeOne = function (token) {
    ...
    if (type === 'text') {
        token = self.tokenizeOne(this.blockToken('paragraph', token.value));
    } else if (type === 'table') {
        index = -1;
...
Parser.prototype.codeBlock = codeBlock;
Parser.prototype.heading = heading;
Parser.prototype.voidToken = voidToken;
Parser.prototype.blockToken = blockToken;
Parser.prototype.link = link;
Parser.prototype.inlineToken = inlineToken;
