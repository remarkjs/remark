...
function tokenizeBlockquote(eat, $0) {
    ...
    add.all(self.token($0.replace(EXPRESSION_BLOCK_QUOTE, '')));
...
function tokenizeFootnoteDefinition(eat, $0, $1, $2, $3) {
    ...
    token.children = self.token($3.replace(EXPRESSION_INITIAL_TAB, ''));
 ...
 */
function BlockLexer(options, shared) {
    var self,
        rules;
 ...
 * @return {Array.<Object>}
 */
BlockLexer.prototype.token = function (value) {
    var self,
        tokens,
        methods,
        index,
...
function Parser(options) {
    ...
    self.block = new BlockLexer(options, shared);
...
Parser.prototype.parse = function (value) {
    ...
    token = {
        'type': 'root',
        'children': self.lex(self.block.lex(value))
    };
