...
function tokenizeBlockquote(eat, top, $0) {
    ...
    /*
     * Pass `top` to keep the current state.
...
function tokenizeFootnoteDefinition(eat, top, $0, $1, $2, $3) {
    ...
    this.footnotes[$2.toLowerCase()] = token.children;
}
...
function tokenizeTag(eat, $0) {
    if (!this.inLink && EXPRESSION_HTML_LINK_OPEN.test($0)) {
        this.inLink = true;
    } else if (this.inLink && EXPRESSION_HTML_LINK_CLOSE.test($0)) {
        this.inLink = false;
    }
...
function tokenizeReferenceLink(eat, $0, $1, $2) {
    ...
    if (
        ...
        if (
            ...
            eat($0.charAt(0))(this.raw('text', $0.charAt(0)));
        }
    } else {
        eat($0)(this.link(
...
function tokenizeInlineCode(eat, $0, $1, $2) {
    eat($0)(this.raw('inlineCode', $2.trimRight()));
}
 ...
 */
Parser.parse = function (value, links, footnotes, options) {
    var parser,
        footnotesAsArray,
        footnote,
        index,
    ...
    parser.inline = new InlineLexer(links, footnotes, parser.options);
...
Parser.prototype.tok = function () {
        ...
        while (++index < length) {
            ...
            while (++cellIterator < cellLength) {
                cells[cellIterator].children =
                    this.inline.token(cells[cellIterator].children);
            }
        }
    ...
    if (type === 'heading' || type === 'paragraph') {
        token.children = this.inline.token(token.children);
    ...
    if (type === 'text') {
        return this.inline.token(this.token.value);
    }
...
function parse(value, options) {
    ...
    return Parser.parse(tokens, lexer.links, lexer.footnotes, options);
}
