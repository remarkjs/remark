...
function tokenizeText(eat, value, silent) {
    ...
    textStopCount = textStop.length;

    while (index < length) {
        character = value.charAt(index);
        position = -1;

        while (++position < textStopCount) {
            stop = textStop[position];
            ...
            if (
                ...
                (
                    stop.length > 1 &&
                    stop === value.slice(index, index + stop.length)
                )
            ) {
                character = EMPTY;
                break;
 ...
 * @param {string} value - Rest of content.
 * @param {boolean?} [silent] - Whether this is a dry run.
 * @return {Node?|boolean} - `escape` node.
 */
    ...
    if (value.charAt(0) === C_BACKSLASH) {
        ...
        if (self.escape.indexOf(character) !== -1) {
            ...
            return eat(C_BACKSLASH + character)(
                self.renderRaw(T_ESCAPE, character)
            );
        }
    }
}
...
Parser.prototype.setOptions = function (options) {
    ...
    var inlineTextStop = self.data.inlineTextStop;
    var textStop = inlineTextStop.default;
    ...
    for (key in defaultOptions) {
        textStop = textStop.concat(inlineTextStop[key] || []);
