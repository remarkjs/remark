    ...
    while (value) {
        ...
        if (cap = this.rules.text.exec(value)) {
            ...
            if (tokens[tokens.length - 1] && tokens[tokens.length - 1].type === 'text') {
                tokens[tokens.length - 1].value += cap[0];
            } else {
                tokens.push({
                    'type' : 'text',
                    'value' : cap[0]
    ...
    if (type === 'listItem') {
        ...
        while (this.next().type !== 'listItemEnd') {
            if (this.token.type === 'text') {
                children.push.apply(children, this.parseText());
            } else {
                children.push(this.tok());
            }
        }
