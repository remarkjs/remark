...
// Create a single source with all parse options turned to their default values.
...
function parseOptions(name) {
  ...
  while (++index < length) {
    ...
    if (key === 'output') {
      ...
      if (key in defaults.stringify && value !== options.stringify[key]) {
...
//
// Virtual options are generated when no fixtures exist, whereas physical
// options are only used whn a file exists.
var virtual = {}
var physical = {}
var all = {}
...
// Get the difference between two objects.  Greatly simplified because `options`
// and `compate` consist solely of booleans, and all properties exist in both.
...
// Find the closest fixture for a `source` in all `fixtures`.  Returns a key of
// a fixture.
...
// Find the closest fixture for all `options`.  Returns an object mapping
// options sources to fixture names.
...
var tests = fs
  ...
  .map(function (filepath) {
    ...
    Object.keys(all).forEach(function (source) {
      ...
      treename = [filename.join('.'), source ? '.' + source : '', '.json'].join(
        ''
      )
    ...
    if (settings.output) {
      if (Object.keys(fixtures).length > 1) {
        throw new Error('Multiple fixtures for output `' + name + '`')
      }
    }
