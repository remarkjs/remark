...
test('remarkStringify', async (t) => {
  ...
  await t.test('should support optional list fields', () => {
    assert.equal(
      toString({
        type: 'list',
        children: [
  ...
  await t.test('should support optional list item fields', () => {
    /** @type {BlockContent[]} */
    const children = [
      {type: 'paragraph', children: [{type: 'text', value: 'alpha'}]},
      {
  ...
  await t.test('should support empty list items', () => {
    assert.equal(toString({type: 'listItem', children: []}), '*\n')
  })

  await t.test('should process references with casing properly', () => {
  ...
  await t.test('should process associations without label', () => {
    assert.equal(
  ...
  await t.test('should stringify mailto links properly', () => {
    assert.equal(
      unified()
        .use(remarkParse)
        .use(remarkStringify)
